...
public class MainActivity extends AppCompatActivity
    ...
    private int[] arr_rssi = new int[15] ;
    Fragment fragment;
    ...
    private int rssi_near = 0;
    private int rssi_far = 0;
    ...
    public static String deviceName = "[ESENS]";
    ...
    public void close() {
        if (mBluetoothGatt == null) {
            Log.e(TAG,"mBluetoothGatt is null(in the close)");
            return;
        }
...
//            RidingListGet(user_id,Year,Month,Day);
           drawPolyLine();
                ...
                if(diff == 0)
                {
                    ...
                    TextView sum_calorie = (TextView) findViewById(R.id.sum_calorie);
                    setCal_kg_m();
                    sum_calorie.setText(Math.round((cal_kg_m * diff / 60*100)/100.0f) + "kcal");
...
/*        if (gara == 1) {
            ...
            new BluetoothAdapter.LeScanCallback() {
                ...
                public void onLeScan(final BluetoothDevice device, final int rssi, byte[] scanRecord) {
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            if (device.getName() != null) { //BLE의 NAME이 ESENS일때만 연결한다.
                                if(device.getName().equals("[ESENS]")){
                                    if (rssi > -60) { //가까워야 연결함
                                        // 단계를 2단계로 해서
                                        if((rssi - rssi_near) < 0 && rssi > -40){

                                            String push_data = "scanTest";
                                            ...
                                            Log.e(TAG, "==================OnLeScan=================");
                                            Log.e(TAG, "거리 가까움, 블루투스 연결 ");
                                            Log.e(TAG, "연결된 DEVICE NAME :  " + device.getName());
                                            Log.e(TAG, "연결된 DEVICE RSSI :  " + rssi);
    ...
    private void connectBLE(int rssi){
        int i=0;
        int cnt=0;
        for(i=0; i<15; i++){
            if(arr_rssi[i] == 0) arr_rssi[i] = rssi;
        }

        if(arr_rssi[14] != 0) {
            for(i = 0; i<15; i++) {
                if (arr_rssi[i] >= arr_rssi[14 - i]) {
                    Log.e(TAG, "사용자 가까워짐!! ");
                    cnt += 1;
                } else {
                    cnt -= 1; //사용자 멀어짐
            ...
            for (i = 0; i < 15; i++) {
                Log.e(TAG, " RSSI ARRAY FULL RSSI [" + i + "] is  " + arr_rssi[i] + "CNT : " + cnt);
                if( cnt > 5) {
                    Log.e(TAG,"사용자 거리 가까워짐 ! CNT 5 이상");
                    mBluetoothAdapter.stopLeScan(mLeScanCallback);

                }else if( cnt < -5 ) {
                    Log.e(TAG,"사용자 거리 멀어짐 ! CNT -5 이하");
                    mBluetoothAdapter.stopLeScan(mLeScanCallback);
    ...
    //BLE에 특정 값을 보내 자물쇠를 잠그거나 해제한다.
    private void lockstate(boolean state){
        Log.e(TAG,"lockstate is called! state is : " + state);
        String push_data;
        ...
        if(state) push_data = "Y";
        else push_data = "N";

        //BLE Service Bind
        //BLE 서비스 bind 해주면 listener가 data available 상태로 변함.
        // 즉 , 서비스를 bind해주고 데이터를 push 해주면 됨.
        ...
        }else Log.e(TAG,"characteristic or mBluetoothGatt is null!!");
    }
    ...
    private final BroadcastReceiver mGattUpdateReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (BluetoothLeService.ACTION_GATT_CONNECTED.equals(action)) {
                ...
                lockstate(lock_state);
                if(lock_state) lock_state = false;
    ...
    //현재 disconnected는 데이터를 한번이상 주고 받았을 때만 원활이 종료됨.
    private void displayData(String send_data){
        //안드로이드 내부에서 순서를 정해서 블루투스와 첫번째 연결이면 ~하는 방식으로 해야함.
        /*String push_data = "o";
        //자물쇠 잠금상태를 뜻한다. true = 알고있음, false = 모르고있음
        if(lock_state == false) push_data = "o"; //상태 전달 받아야함
        else push_data = "n";   //상태 전달 받은 상태

       // if(send_data.equals("AEAO")){
            Log.e(TAG,"자물쇠 열려 있는 상태!");
       ...
       // }
        ...
        Log.e(TAG,"DATA IS :" + send_data);
    }

    //RSSI값 측정을 위해 BLE 연결 및 연결 해제를 하는 부분
    ...
    private class LeDeviceListAdapter extends BaseAdapter {
        ...
        public void addDevice(BluetoothDevice device) {
            Log.e(TAG,"mLeDevices is called!");
...
//                mDatabase.child("users").child("TEST").child(Year).child(Month).child(Day).setValue(myRiding.list.get(0).time);
        ...
        btn.setOnClickListener(new View.OnClickListener()  {
            ...
            public void onClick(View v) {
                ...
                if(diff == 0)
                {
                    ...
                    sum_calorie.setText(Math.round((cal_kg_m * diff / 60)*100d)/100d + "kcal");
                ...
                if(mBluetoothLeService != null) mBluetoothLeService = null;
                close();
                //while(lock_state == true) scanLeDevice(true);
                mBluetoothAdapter.startLeScan(mLeScanCallback);

                if(lock_state) mBluetoothAdapter.startLeScan(mLeScanCallback);
    ...
    public void RidingListGet(final String user_id, final String year, final String month, final String day) {
        ...
        if (user_id != null && year != null && month != null && day != null) {
